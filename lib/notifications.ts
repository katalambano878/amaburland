...
const ADMIN_EMAIL = process.env.ADMIN_EMAIL || 'diyabeauty1@yahoo.com';
const EMAIL_FROM = process.env.EMAIL_FROM || "DIYA'S ORGANICS <noreply@example.com>";
const BRAND = {
...
`, `Welcome to ${BRAND.name}, ${firstName}!`)
    ...
    if (phone) {
        await sendSMS({
            to: phone,
            message: `Welcome ${firstName}! Thanks for joining our store.`
        });
    }
}
