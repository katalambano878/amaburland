// Amaburland - network-first pages; no HTML shell cache (playbook section 16)
const CACHE_VERSION = 'sw-v2.5-amaburland';
...
self.addEventListener('push', (event) => {
  ...
  const options = {
    body: data.body || 'New update from Amaburland',
  ...
  event.waitUntil(
    self.registration.showNotification(data.title || 'Amaburland', options)
  );
});
